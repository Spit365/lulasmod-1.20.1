...
public class ParticleProjectileEntity extends PersistentProjectileEntity {
    ...
    public ParticleProjectileEntity(World world, LivingEntity owner, Vec3d pos, Vec3d velocity, @Nullable ParticleEffect particleEffect) {
        super(ModClient.Entities.PARTICLE_PROJECTILE, owner, world, owner.getOffHandStack(), owner.getMainHandStack());
    ...
    public void readCustomDataFromNbt(NbtCompound nbt) {
        ...
        if (nbt.contains("Particle", NbtElement.STRING_TYPE)) {
            try {
                this.particleEffect = ParticleEffectArgumentType.readParameters(new StringReader(nbt.getString("Particle")), (RegistryWrapper.WrapperLookup) Registries.PARTICLE_TYPE.getReadOnlyWrapper());
