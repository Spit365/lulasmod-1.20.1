...
import net.spit365.lulasmod.custom.entity.model.TailModel;
...
public class TailFeatureRenderer extends FeatureRenderer<AbstractClientPlayerEntity, PlayerEntityModel<AbstractClientPlayerEntity>> {
	...
	public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, AbstractClientPlayerEntity abstractClientPlayerEntity, float f, float g, float h, float j, float k, float l) {if (
			TAILED_PLAYER_LIST.contains(abstractClientPlayerEntity.getUuidAsString())
			&& !abstractClientPlayerEntity.isInvisible())
		{
			...
			matrixStack.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(-o));
			TailModel.getAnimatedTexturedModelData().createModel().render(matrixStack, vertexConsumer, i, LivingEntityRenderer.getOverlay(abstractClientPlayerEntity, 0f));
			matrixStack.pop();
		}
	}
}
