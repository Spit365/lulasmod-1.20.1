...
public abstract class SealItem extends Item {
    ...
    protected static Boolean spellSelected(PlayerEntity player, Item item) {
        return item instanceof IncantationItem && TagManager.readList(Lulasmod.MOD_ID, player, ModTagCategories.SPELLS).get(0).equals(Registries.ITEM.getId(item).getPath());
    }
        ...
        if (!world.isClient() && canUse(player)) {
            ...
            if (spellSelected(player, ModItems.DASH_INCANTATION)) {
                if (!player.hasStatusEffect(StatusEffects.SLOWNESS)) {
                    String read = TagManager.read(Lulasmod.MOD_ID, player, ModTagCategories.DASH_SPELL);
                    if (read == null)
                        TagManager.put(Lulasmod.MOD_ID, player, ModTagCategories.DASH_SPELL, String.valueOf(5 * cooldownMultiplier()));
                    if (read.equals("1")) {
                        player.getItemCooldownManager().set(this, 50);
                        TagManager.put(Lulasmod.MOD_ID, player, ModTagCategories.DASH_SPELL, String.valueOf(5 * cooldownMultiplier()));
                    } else {
                        player.getItemCooldownManager().set(this, 2);
                        TagManager.put(Lulasmod.MOD_ID, player, ModTagCategories.DASH_SPELL, String.valueOf(Math.min(5 * cooldownMultiplier(), Integer.parseInt(read)) - 1));
                    }
            ...
            if (TagManager.readList(Lulasmod.MOD_ID, player, ModTagCategories.SPELLS).get(0) != null){
