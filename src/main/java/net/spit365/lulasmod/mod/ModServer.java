...
public class ModServer {
     ...
     private static class register {
          ...
          private static RegistryEntry<StatusEffect> StatusEffect(String name, StatusEffect entry) {
               Identifier id = Identifier.of(Server.MOD_ID, name);
               return RegistryEntry.of(Registry.register(Registries.STATUS_EFFECT, id, entry));
          }
     ...
     public static class Items {
         ...
         public static final List<Item> tailedExclusive = List.of(Items.HELLISH_SEAL, Spells.SLASH_SPELL, Spells.BLOOD_SPELL, Spells.POCKET_SPELL);
          ...
          public static final SpellItem POCKET_SPELL = register.Spell("heresies", new SpellItem(0, ENTITY_ZOMBIE_VILLAGER_CURE) {@Override public void cast(ServerWorld world, PlayerEntity player, Hand hand, Float efficiencyMultiplier, Integer cooldownMultiplier) {
              ...
              for (Entity victim : entities) {
                 ...
                 if (world.getRegistryKey().equals(World.OVERWORLD) && victim instanceof ServerPlayerEntity serverPlayer){
                    ServerPlayNetworking.send(serverPlayer, new ModPackets.TIME_FORWARD_ANIMATION_S2C());
     ...
     public static class StatusEffects {
         public static final RegistryEntry<StatusEffect> CUSHIONED = register.StatusEffect("cushioned", new CushionedStatusEffect());
         public static final RegistryEntry<StatusEffect> BLEEDING = register.StatusEffect("bleeding", new BleedingStatusEffect());
     ...
     public static class Tickers {
          ...
          private static void sendSpellListPacket(ServerPlayerEntity player, List<Identifier> list) {
               ServerPlayNetworking.send(player, new ModPackets.SPELL_HOTBAR_LIST_S2C(list));
          }
               ...
               for (ServerPlayerEntity player : input.getPlayerManager().getPlayerList()) {
                    if (player.getCommandTags().contains("miner")) {
                         ...
                         if (closestPortal != null) {
                              ModMethods.outlineBox(new Box(closestPortal.getX() +5, closestPortal.getY() +5, closestPortal.getZ() +5, closestPortal.getX() -5, closestPortal.getY() -5, closestPortal.getZ() -5), player.getServerWorld(), ModClient.Particles.GOLDEN_SHIMMER);
               ...
               for (ImpaledContext context : impaled) {
                    ...
                    if (context.iterations > 0 && victim.isAlive()) {
                         ...
                         if (impaledCounter >= 25) {
                              ...
                              TagManager.put(context.player, TagCategories.DAMAGE_DELAY, Identifier.of(Server.MOD_ID, "0"));
                         ...
                         impaled.remove(context);
                         TagManager.remove(context.player, TagCategories.DAMAGE_DELAY);
          ...
          public static final TickerManager.Ticker<MinecraftServer> updateTailedVisuals = TickerManager.createTicker(MinecraftServer.class, input -> {
               ...
               for (ServerPlayerEntity player : input.getPlayerManager().getPlayerList())
                    ServerPlayNetworking.send(player, new ModPackets.TAILED_PLAYER_LIST_S2C(tailedPlayers));
          });
