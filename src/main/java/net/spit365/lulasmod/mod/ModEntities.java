...
public class ModEntities {
    ...
    public static final EntityType<ParticleProjectileEntity> PARTICLE_PROJECTILE = register(
        "particle_projectile",
        EntityType.Builder.<ParticleProjectileEntity>create(ParticleProjectileEntity::new, SpawnGroup.MISC)
                .setDimensions(0.5F, 0.5F)
                .maxTrackingRange(4)
                .trackingTickInterval(20),
        ParticleProjectileEntityRenderer::new
    );
    private static <T extends Entity> EntityType<T> register(String id, EntityType.Builder<T> type, EntityRendererFactory<T> entityRendererFactory) {
         EntityType<T> entityType = Registry.register(Registries.ENTITY_TYPE, new Identifier(Lulasmod.MOD_ID, id), type.build(new Identifier(Lulasmod.MOD_ID, id).toString()));
